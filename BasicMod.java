...
public class BasicMod implements
    ...
    public static Boolean OrigStartDeckSetting = Boolean.valueOf(false);
    public static Boolean EnColorlessSetting = Boolean.valueOf(false);
    public static Boolean OrigStartDeck = Boolean.valueOf(false);
    public static Boolean EnColorless = Boolean.valueOf(false);


    public void receivePostDungeonInitialize() {
        if (OrigStartDeckSetting.booleanValue()) OrigStartDeck = true;
        if (EnColorlessSetting.booleanValue()) EnColorless = true;
    }
    ...
    public void receivePostInitialize() {
        ...
        ModLabeledToggleButton enableOriginalStartingDeck = new ModLabeledToggleButton("Enable the Prismatic's original, more powerful starting deck?", 350.0F, 750.0F, Settings.CREAM_COLOR, FontHelper.charDescFont, OrigStartDeckSetting.booleanValue(), settingsPanel, label -> {
        }, button -> SetBoolean(Boolean.valueOf(button.enabled))
        );
        settingsPanel.addUIElement((IUIElement) enableOriginalStartingDeck);

        ModLabeledToggleButton enableColorlessCards = new ModLabeledToggleButton("Enable the Prismatic to see colorless cards in card rewards?", 350.0F, 700.0F, Settings.CREAM_COLOR, FontHelper.charDescFont, EnColorlessSetting.booleanValue(), settingsPanel, label -> {
        }, button -> SetBoolean(Boolean.valueOf(button.enabled))
        );
        settingsPanel.addUIElement((IUIElement) enableColorlessCards);
